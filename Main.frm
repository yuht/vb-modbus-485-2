...
Private Sub cmdCommand2_Click()
    Timer_GetData.Enabled = Not Timer_GetData.Enabled
    cmdCommand2.Caption = IIf(Timer_GetData.Enabled = True, "正在运行", "已经停止")
End Sub
...
'        End If
        ...
        'Debug.Print "____",
        For j = 0 To UBound(intA())
            Text1 = Text1 & "_" & Right("00" & Hex(intA(j)), 2)
        Next
        Text1 = Text1 & vbCrLf
...
Private Sub Timer_GetData_Timer()
    ...
    For i = DeviceNumber To &HFF
        If MSComm1.PortOpen = True Then
                ModBus485Data(1) = DeviceNumber
                Call CRC16(ModBus485Data, 0, 3, ModBus485Data(4), ModBus485Data(5))
                'MSComm1.RThreshold = 16
                MSComm1.Output = ModBus485Data()
                
                Debug.Print "send" & DeviceNumber & ":",
                
                For j = 0 To UBound(ModBus485Data)
                    Debug.Print Hex(ModBus485Data(j)),
                Next
                Debug.Print
                If DeviceNumber <> &HFF Then
